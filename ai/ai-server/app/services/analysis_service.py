...
# - posture/face/gaze 모두 전 프레임(stride=1)
...
class VideoFrameIterator:
    ...
    def __iter__(self):
        ...
        printed = 0
        # OpenCV (CPU 디코드) — 분석 단계에선 결국 numpy로 가져와야 하므로 CPU 프레임이 필요합니다.
        try:
            import cv2
        ...
        # PyAV fallback (여기도 결국 CPU로 올려야 numpy가 됩니다)
        try:
